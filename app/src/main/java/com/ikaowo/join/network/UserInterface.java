...
public interface UserInterface {
  @POST("user/login") Call<SignupResponse> signin(@Body LoginRequest request);

  @POST("user/signup") Call<SignupResponse> signup(@Body SignupRequest request);

  //忘记密码重置
  @PUT("user/fgpw") Call<BaseResponse> resetPasswd(@Body ResetPasswdRequest request);

  //修改
  @PUT("user/password") Call<BaseResponse> updatePasswd(@Body UpdatePasswordRequest request);

  @PUT("user/user_info") Call<BaseResponse> updateAvatar(@Body UpdateAvatarRequest request);

  @PUT("user/session") Call<CheckStateResponse> checkLatestState(@Body CheckStateRequest request);
}
