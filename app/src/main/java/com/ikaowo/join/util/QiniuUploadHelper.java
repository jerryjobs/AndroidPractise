...
public class QiniuUploadHelper {
  ...
  public void uploadImg(Context context, final int requestCode, final int resultCode,
      final Intent data, final PhotoService.UploadFinishListener listener) {
    ...
    networkManager.async(context, Constant.PROCESSING, call,
        new KwMarketNetworkCallback<TokenResponse>(context) {
          @Override public void onSuccess(final TokenResponse tokenResponse) {
            imageServerUrl = tokenResponse.url;
            photoService.onUploadPic(imageServerUrl, requestCode, resultCode, tokenResponse.key,
                tokenResponse.token, data, listener);
          }
        });
  }
}
