...
public class RecyclerViewHelper<T extends JResponse, P> {
  ...
  //step #1
  public void init(Context context, final ScrollMoreRecyclerView recyclerView, JAdapter adapter,
    ...
    this.listener = new GetDataListener() {
      @Override public void onStart(Action action) {
        startRefresh(true);
      ...
      callback = new NetworkCallback<T>(context) {
        @Override public void onSuccess(T t) {
          ...
          recyclerView.enableLoadMore(
              ((cp * ps) + helperInterface.getList(t).size()) < t.getTotals());
          onFinishLoadData();
