...
public class ImageLoader {
  ...
  public void loadImage(ImageView imageView, String url, int targetWidth, Target target,
      int defaultImg) {
    int mode = 1;
    load(context, imageView, url, targetWidth, -1, mode, target, defaultImg);
  }

  public void loadImage(ImageView imageView, String url, int targetWidth, int targetHeight,
      int defaultImg) {
    load(context, imageView, url, targetWidth, targetHeight, 1, null, defaultImg);
  }

  public void loadImage(ImageView imageView, String url, int targetWidth, int targetHeight,
      Target target, int defaultImg) {
    load(context, imageView, url, targetWidth, targetHeight, 1, target, defaultImg);
  }

  public void loadImage(ImageView imageView, String url, int targetWidth, int targetHeight,
      int mode, int defaultImg) {
